...
pwd=$(pwd)
echo -e "\033[36m==="
echo "download u-boot_OPI-emmc.fex & boot0_OPI.fex"
echo -e "===\033[37m"
wget https://github.com/loboris/OrangePi-BuildLinux/blob/master/orange/u-boot_OPI-emmc.fex?raw=true > /dev/null 2>&1
wget https://github.com/loboris/OrangePi-BuildLinux/blob/master/orange/boot0_OPI.fex?raw=true > /dev/null 2>&1
...
if [ ! -f boot0_OPI.fex ]; then
    echo "Error: boot0_OPI.fex not found."
    exit 1
fi
if [ ! -f u-boot_OPI-emmc.fex ]; then
...
#************************************************************************
echo ""
echo "Instaling boot0 & u-boot to EMMC ..."
dd if=boot0_OPI.fex of=${sdcard} bs=1k seek=8 > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "ERROR installing boot0."
    exit 1
fi
dd if=u-boot_OPI-emmc.fex of=${sdcard} bs=1k seek=16400 > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
#-----------------------------------------------------------------------------------------
cp -a /media/boot/ $bootdir/ > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "  ERROR."
fi
#-----------------------------------------------------------------------------------------
